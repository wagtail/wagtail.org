...
def deploy_production():
    deploy_to_heroku(
        PRODUCTION_APP_INSTANCE, local_branch="main", remote_branch="master"
    )
...
def clean_local_database(local_database_name=LOCAL_DATABASE_NAME):
    local(
        ...
        'CASCADE; CREATE SCHEMA public;"'.format(database_name=local_database_name)
    )
...
def deploy_prompt(app_instance):
    prompt_msg = (
        "You are about to do a manual deployment. \nPlease type "
...
def get_heroku_variable(app_instance, variable):
    return local(
        "heroku config:get {var} --app {app}".format(
            app=app_instance,
            var=variable,
        ),
        capture=True,
...
def pull_media_from_s3_heroku(app_instance):
    ...
    aws_secret_access_key = get_heroku_variable(app_instance, "AWS_SECRET_ACCESS_KEY")
    aws_storage_bucket_name = get_heroku_variable(
        app_instance, "AWS_STORAGE_BUCKET_NAME"
    )
    pull_media_from_s3(
...
def push_media_to_s3_heroku(app_instance):
    ...
    push_media_to_s3(aws_access_key_id, aws_secret_access_key, aws_storage_bucket_name)
...
def deploy_to_heroku(app_instance, local_branch="master", remote_branch="master"):
    print(
    ...
    local(
        "git push {remote} {local_branch}:{remote_branch}".format(
            remote=remote_name,
